...
class CalendarAPI:
    ...
    def refresh(self, ttl=None):
        ...
        live_timeslots = self.get_slots_from_cal(
            self.client.calendar(url=config.url + "/calendars/" + config.user + "/emiter_audycje"),
            d_from,
            d_to,
        )
        replay_timeslots = self.get_slots_from_cal(
            self.client.calendar(url=config.url + "/calendars/" + config.user + "/emiter_powtorki"),
            d_from,
            d_to,
        )

        logging.info(
            "found %d live shows and %d replays." % (len(live_timeslots), len(replay_timeslots))
        )
        ...
        for slot in live_timeslots:
            ...
            if type(name) is not str:
                logging.warning(
                    "program '%s' does not have name defined. Using default (%s)" % (slug, slug)
                )
                name = slug

            if type(rds) is not str:
                logging.warning(
                    "program '%s' does not have RDS defined. Using default (%s)" % (slug, name)
                )
                rds = name
